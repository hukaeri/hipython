...
original_author="hukaeri"
original_name="hipython"
original_urlname="hipython"
original_description="Awesome hipython created by hukaeri" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/hipython "${name}"
...
then
    ...
    echo "then run 'hipython' or 'python -m hipython'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'hipython' or 'python -m hipython' or 'poetry run hipython'"
fi
