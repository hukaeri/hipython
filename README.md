...
- 🎯 Entry points to execute your program using `python -m <hipython>` or `$ hipython` with basic CLI argument parsing.
...
---
# hipython Flask Application

Awesome hipython created by hukaeri
...
```bash
git clone https://github.com/hukaeri/hipython hipython
cd hipython
make install
```
...
```bash
hipython create-db   # run once
hipython populate-db  # run once (optional)
hipython add-user -u admin -p 1234  # ads a user
hipython run
```
